...
dojo.declare("OfflajnSliderSimpleHorizontal", null, {
    ...
    this.width = parseInt(dojo.position(this.lis[0]).w);
    
    this.previous = dojo.query('.left', this.node);
    if(this.previous[0])
      dojo.connect(this.previous[0], 'onclick', dojo.hitch(this, 'onBtn', -1) );
    
    this.next = dojo.query('.right', this.node);
    if(this.next[0])
      dojo.connect(this.next[0], 'onclick', dojo.hitch(this, 'onBtn', 1) );
  ...
  onAnimateSliding: function(e){
    var px = parseInt(e);
    dojo.style(this.ul, 'marginLeft', px+'px');
  },
