...
<?php echo $c['id']; ?> .shadow{
  width: <?php echo $cwidth; ?>px;
  background: url('<?php if(!$this->calc && $this->env->slider->params->get('shadow') != '-1' && $this->env->slider->params->get('shadow') != '') echo $this->themeCacheUrl.$c['helper']->ResizeImage(dirname(__FILE__).'/../../../images/shadows/'.$this->env->slider->params->get('shadow'), $cwidth, 0); ?>') no-repeat;
...
<?php echo $c['id']; ?> .controll .right:hover, <?php echo $c['id']; ?> .controll .left:hover{
  background: #<?php echo substr($this->env->slider->params->get('selectedtabbg'),0,6); ?> url('<?php echo $c['url']; ?>images/pattern.png') repeat;
}
...
<?php echo $c['id']; ?> .controll .dots .dot.selected{
  background-image: url('<?php if(!$this->calc) echo $this->themeCacheUrl.$c['helper']->ColorizeImage(dirname(__FILE__).'/images/dotselected.png', $this->env->slider->params->get('tabbg'), '188DD9'); ?>');
}
